...
const convertOne = async function () {
  ...
  result["mainIngredient"] = result.mainIngredient.mainIngredientList.map(
    (item) => new Ingredient(item)
  );
  result["accessories"] = result.accessories.accessoriesList.map(
    (item) => new Ingredient(item)
  );
  result["measure"] = result.measure.measureList.map(
    (item) => new Measure(item)
  );
